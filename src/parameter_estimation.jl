...
function objective_function(parameter_guess_array,time_start,time_step_size,time_stop,model_data_dictionary,exp_data_dictionary)
    ...
    error_vector_2a = protein_Venus_exp .- protein_Venus_sim
    ...
    error_vector_2b= 1*protein_Venus_exp .- protein_GntR_sim
   
    error_term_array[2] = transpose(error_vector_2a)*error_vector_2a + transpose(error_vector_2b)*error_vector_2b
    # error_term_array[2] = transpose(error_vector_2a)*error_vector_2a
...
function check_parameter_bounds(parameter_array)
    ...
    pvec_bounds = [
        ...
        0.001 1.0 	    ;	# 20	    protein_sigma_70
         ...
         # w -
         2.0 15.0           ;   # 21    translation capacity half-life
...
pvec_initial = [
	...
	1.0    	;   # 3     W_Venus_RNAP
	-1.0    ;   # 4     W_Venus_sigma_70
	-5.0    	;   # 5     W_Venus_GntR

	# binding parameters -
	2.5            ;   # 6     n_GntR_sigma_70
	30         ;   # 7     K_GntR_sigma_70
	2.5            ;   # 8     n_Venus_sigma_70
	100         ;   # 9     K_Venus_sigma_70
	2.5            ;   # 10     n_Venus_GntR
	1         ;   # 11    K_Venus_GntR
	...
	1.0 	    ;	# 20	    protein_sigma_70
	 ...
	 # w -
	 4.0           ;   # 21    translation capacity half-life
...
# execute -
number_of_trials = 12 # specify number of trials here
for trial_index = 1:number_of_trials
   ...
   # do a local step -
    if (mod(trial_index,2) == 0)
        ...
        # local refine -
        pV = local_refienment_step(path_to_data_dir, pV_best; iteration_max=30)
    end

    # main -
    (EC,PC,RA) = main(path_to_data_dir, vec(pV); rank_cutoff=5,maximum_number_of_iterations=30)

    # dump results to disk -
    fname = "./src/simulated/poets_ensemble/RA_T$(trial_index).dat"
    writedlm(fname,RA)
    fname = "./src/simulated/poets_ensemble/EC_T$(trial_index).dat"
    writedlm(fname,EC)
    fname = "./src/simulated/poets_ensemble/PC_T$(trial_index).dat"
    writedlm(fname,PC)
